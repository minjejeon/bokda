...
class LogisticRegressionFreq:
    ...
    def __init__(self, method: str = 'newton', tolerance: float = 1e-6, ltolerance: float = 1e-7,
                 max_iter: int = 300, verbose: bool = False):
        """
        로지스틱 회귀 모델을 초기화
        ...
        ----------
        method : str, optional
            최적화 방법 설정 | Default: 'newton'
        tolerance : float, optional
            계수 벡터의 수렴 기준(tolerance) | Default: 1e-6
        ltolerance : float, optional
            로그 우도 함수의 수렴 기준 | Default: 1e-7
        max_iter : int, optional
            최대 반복 횟수 | Default: 300
        verbose : bool, optional
            과정을 출력할 지 여부 | Default: False
        """
        self.method = method
        self.tolerance = tolerance
        self.ltolerance = ltolerance
        ...
        if self.verbose:
            ...
            print(f"  - Log Likelihood Tolerance: {self.ltolerance}")
    ...
    def fit(self, X: Union[np.ndarray, pd.DataFrame],
            ...
            columns: List[str] = [], add_const: bool = True) -> 'LogisticRegressionFreq':
        """
        주어진 데이터로 로지스틱 회귀 모델을 적합
        ...
        if self.method == 'newton':
            self._fit_newton(X, y, tolerance=self.tolerance, ltolerance=self.ltolerance, max_iter=self.max_iter)
        else:
    ...
    def _check_input_array(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        ------
        ValueError
            X, y가 numpy array가 아니거나, 차원이 맞지 않거나, NaN 값이 포함된 경우 발생
        """
        # numpy array 인지 확인
    ...
    def _calculate_gradient(self, X: np.ndarray, y: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
        """
        ...
        gradient : numpy array of shape (n_features,)
            기울기 벡터
        """
        gradient = X.T @ (y_pred - y)  # X^T * (y_hat - y)
        return gradient

    def _calculate_hessian(self, X: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
        """
    ...
    def _fit_newton(self, X: np.ndarray, y: np.ndarray, tolerance: float, ltolerance: float, max_iter: int) -> None:
        """
        뉴턴-랩슨 방법으로 모델을 적합
        ...
        last_log_likelihood = -np.inf  # 초기 로그 우도 값을 매우 작은 값으로 설정

        start_time = time.time()
        for iteration in range(max_iter):
            # 선형 결합 계산
            linear = X @ self.beta  # @: 행렬곱

            # 시그모이드 함수 적용하여 예측 확률 계산
            y_pred = 1 / (1 + np.exp(-linear))  # 1 / (1 + e^(-X @ beta))

            # 기울기와 헤시안 행렬 계산
            gradient = self._calculate_gradient(X, y, y_pred)
            hessian = self._calculate_hessian(X, y_pred)
            ...
            # 계수 수렴 여부 확인
            is_converged_tol = np.linalg.norm(delta, ord=1) <= tolerance
            if is_converged_tol:
                # PRINT: 몇 번째 iteration에서 수렴했는지
                if self.verbose:
                    elapsed_time = time.time() - start_time
                    print(f"> Converged by tolerance at iteration {iteration + 1} / {self.max_iter} (elapsed: {elapsed_time:.4f}s)")
                break

            # 로그 우도 수렴 여부 확인
            log_likelihood = np.sum(y * np.log(y_pred + 1e-9) + (1 - y) * np.log(1 - y_pred + 1e-9))
            is_converged_ltol = np.abs(log_likelihood - last_log_likelihood) < ltolerance
            if is_converged_ltol:
                # PRINT: 몇 번째 iteration에서 수렴했는지
                if self.verbose:
                    elapsed_time = time.time() - start_time
                    print(f"> Converged by ltolerance at iteration {iteration + 1} / {self.max_iter} (elapsed: {elapsed_time:.4f}s)")
                break
            last_log_likelihood = log_likelihood  # update

        # max_iter 만큼 반복할 때까지 수렴하지 않은 경우
        else:
    ...
    def _calculate_summary_stats(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        """
        # 예측 확률 계산
        y_pred = 1 / (1 + np.exp(-(X @ self.beta)))  # 예측 확률 벡터
        ...
        dof_residual = n_samples - n_features

        # AIC (Akaike Information Criterion) 계산
        aic = 2 * n_features - 2 * log_likelihood  # 상수항 포함이므로
        ...
        # 각 계수의 표준 오차 계산
        hessian = self._calculate_hessian(X, y_pred)
        covariance_matrix = np.linalg.inv(hessian)  # 공분산 행렬
