...
class LogisticRegressionBayes:
    def __init__(self, mcmcsize: int, n_chains: int = 1, method: str = 'adaptiveMCMC', thinning: int = 1,
        ...
        prior : str, optional
            ...
            NOTE: 현재까지 'weaklyinformative', 'normal_unknown_sigma', 'normal_known_sigma' 지원함.
        ...
        # Initialize beta and accept lists for multiple chains
        ...
        self.sigma = [None] * self.n_chains  # For 'normal_unknown_sigma' prior

        # Initialize trueindex to keep track of sampled indices per chain
    ...
    def fit(self, X: Union[np.ndarray, pd.DataFrame],
            ...
            ) -> 'LogisticRegressionBayes':
        """
        주어진 데이터로 베이지안 로지스틱 회귀 모델을 적합
        ...
        if isinstance(X, pd.DataFrame):
            X = X.to_numpy()  # numpy array로 변환 / NOTE: df.values 보다 df.to_numpy()가 더 안전하고 권장되는 방식
        ...
        for chain_idx in range(self.n_chains):
            if self.method == 'MCMC':
                self._fit_MCMC(X, y, self.jumping_rate, self.prior_alpha, self.prior_beta, chain_idx)
            ...
            if multichain:
                for chain_idx, chain in enumerate(self.beta):
                    if burn_in >= chain.shape[0]:
                        raise ValueError(f"`burn_in` ({burn_in}) exceeds the number of available samples ({chain.shape[0]}). Chain {chain_idx+1}")
                    sns.lineplot(data=chain[burn_in:self.trueindex[chain_idx], i], label=f'Chain {chain_idx+1}', alpha=0.6)
            else:
                for chain_idx, chain in enumerate(self.beta):
                    if burn_in >= chain.shape[0]:
                        raise ValueError(f"`burn_in` ({burn_in}) exceeds the number of available samples ({chain.shape[0]}). Chain {chain_idx+1}")
                    sns.lineplot(data=chain[burn_in:self.trueindex[chain_idx], i], label=f'Chain {chain_idx+1}' if self.n_chains > 1 else None)
        ...
        for i in range(num_beta):
            ...
            if multichain:
                for chain_idx, chain in enumerate(beta_chain):
                    ...
                    sns.histplot(chain_samples, kde=True, color=color_rgb, alpha=0.3)

                    plt.axvline(hdi_lower, color=color_rgb, linestyle='--')
                    plt.axvline(hdi_upper, color=color_rgb, linestyle='--')

                    handles.append(plt.Line2D([0], [0], color=color_rgb, linestyle='--', label=f'Chain {chain_idx + 1} HPD'))
            else:
                # Single chain case
                for chain_idx, chain in enumerate(beta_chain):
                    chain_samples = chain[burn_in:self.trueindex[chain_idx], i]
    ...
    def acf_plot(self, beta_chain: Union[np.ndarray, List[np.ndarray]] = None, burn_in: int = 0, multichain: bool = False,
                 max_lag: int = 40) -> None:
        """
        ...
        Parameters
        ----------
        beta_chain : numpy array of shape (mcmcsize, n_features) or list of 1D/2D arrays, optional
    ...
    def _check_input_array(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        ------
        ValueError
            X, y가 numpy array가 아니거나, dim이 맞지 않으면 에러가 발생
        """
        # numpy array 인지 확인
    ...
    def _standardize_binary_continous(self, data: np.ndarray, desired_mean: float = 0,
        ...
        continuous_columns = ~binary_columns  # continous 변수
    ...
    def _fit_MCMC(self, X: np.ndarray, y: np.ndarray,
                  jumping_rate: float, prior_alpha: float, prior_beta: float, chain_idx: int) -> None:
        """
        주어진 데이터로 MCMC 체인을 돌림
        ...
        None
        """
        # Set random seed for reproducibility per chain
        np.random.seed(self.seed + chain_idx if self.seed is not None else None)
        N, C = X.shape
        beta = np.zeros((self.mcmcsize + 1, C))
        accept = np.zeros(C)
        beta_slack = beta[0].copy()
        i = 0
        pseudo_index = 0

        # MCMC with weakly informative prior
        if self.prior == 'weaklyinformative':
            '''
            Citation of
            Andrew Gelman, Aleks Jakulin, Maria Grazia Pittau, Yu-Sung Su 
            "A weakly informative default prior distribution for logistic and other regression models," 
            The Annals of Applied Statistics, Ann. Appl. Stat. 2(4), 1360-1383, (December 2008)
            '''
            if self.verbose:
            ...
            for pseudo_index in iters:
                if pseudo_index % self.thinning == 0:
                    for j in range(C):
                        nu_beta = beta[i].copy()
                        nu_beta[j] = np.random.normal(beta[i, j], jumping_rate)

                        # Calculate log likelihood for current beta_slack
                        linear_combination = np.dot(X, beta_slack)
                        prop = 1 / (1 + np.exp(-linear_combination))  # Logistic link
                        log_likelihood = np.sum(st.binom.logpmf(y, 1, prop))

                        posterior = log_likelihood + st.cauchy.logpdf(beta_slack[j], loc=0, scale=10)

                        # Calculate log likelihood for proposed nu_beta
                        linear_combination_proposed = np.dot(X, nu_beta)
                        prop_proposed = 1 / (1 + np.exp(-linear_combination_proposed))
                        nu_log_likelihood = np.sum(st.binom.logpmf(y, 1, prop_proposed))
            ...
            self.accept[chain_idx] = accept / self.mcmcsize
            self.trueindex[chain_idx] = self.mcmcsize

        # MCMC with normal prior with unknown sigma
        elif self.prior == 'normal_unknown_sigma':
            ...
            for pseudo_index in iters:
                # for pseudo_index in range(self.mcmcsize):
                if pseudo_index % self.thinning == 0:
                    for j in range(C):
                        nu_beta = beta[i].copy()
                        nu_beta[j] = np.random.normal(beta[i, j], jumping_rate)
                        ...
                        posterior = log_likelihood + st.norm.logpdf(beta_slack[j],
                    ...
                    # Sigma 업데이트
                    for k in range(C):
                        sigma[i + 1, k] = st.invgamma.rvs(N / 2 + prior_alpha,
                                                          scale=np.sum((y - 0.5) ** 2) / 2 + prior_beta + sys.float_info.epsilon)
                    i += 1
                else:
                    for j in range(C):
                        nu_beta = beta[i].copy()
                    ...
                    # Sigma 업데이트
                    for k in range(C):
                        sigma[i, k] = st.invgamma.rvs(N / 2 + prior_alpha,
                                                      scale=np.sum((y - 0.5) ** 2) / 2 + prior_beta + sys.float_info.epsilon)
            # 결과 저장
            self.beta[chain_idx] = beta
            self.sigma[chain_idx] = sigma
            self.accept[chain_idx] = accept / self.mcmcsize
            self.trueindex[chain_idx] = self.mcmcsize

        # MCMC with normal prior with known sigma
        elif self.prior == 'normal_known_sigma':
            if self.verbose:
                # PRINT: verbose = True일 경우에만 progress bar 출력
                iters = tqdm(range(self.mcmcsize), desc=f"Chain {chain_idx + 1}")
            ...
            for pseudo_index in iters:
                ...
                if pseudo_index % self.thinning == 0:
                    for j in range(C):
                        ...
                        posterior = log_likelihood + st.norm.logpdf(beta_slack[j],
                                                                    self.prior_location[j],
                                                                    self.prior_scale[j] + sys.float_info.epsilon)
            ...
            raise ValueError('지원하지 않는 prior입니다.')
    ...
    def _fit_adaptiveMCMC_weaklyinformative(self, X: np.ndarray, y: np.ndarray, acceptance_rate: float,
        ...
        if self.verbose:
            # verbose가 True일 경우에만 progress bar 출력
            iters = tqdm(range(self.mcmcsize), desc=f"AdaptiveMCMC Weakly Informative Chain {chain_idx + 1}")
        else:
            iters = range(self.mcmcsize)
        ...
        for i in iters:
            for j in range(C):
                nu_beta = beta[i].copy()
                # nu_beta[j] = np.random.normal(beta[i, j], np.sqrt(np.exp(np.log(prior_sigma[j]) + prior_lambda[j])))
                nu_beta[j] = np.random.normal(beta[i, j],
                                              np.sqrt(np.exp(np.log(prior_sigma[j] + 1e-8) + prior_lambda[j])))  # log(0) 방지

                # Calculate log likelihood for current beta_slack
                linear_combination = np.dot(X, beta[i])
                prop = 1 / (1 + np.exp(-linear_combination))  # Logistic link
                log_likelihood = np.sum(st.binom.logpmf(y, 1, prop))
    ...
    def _fit_adaptiveMCMC_normal_unknown_sigma(self, X: np.ndarray, y: np.ndarray, acceptance_rate: float,
        ...
        if self.verbose:
            # verbose가 True일 경우에만 progress bar 출력
            iters = tqdm(range(self.mcmcsize), desc=f"AdaptiveMCMC Normal Unknown Sigma Chain {chain_idx + 1}")
        else:
            iters = range(self.mcmcsize)
        ...
        for i in iters:
            ...
            # Update sigma after updating all betas
            for k in range(C):
                # In a logistic regression, updating sigma might not be straightforward.
                # This is a placeholder and may require a more appropriate update mechanism.
                sigma[i + 1, k] = st.invgamma.rvs(N / 2 + prior_alpha,
                                                  scale=np.sum((y - 0.5) ** 2) / 2 + prior_beta + sys.float_info.epsilon)
            i += 1
    ...
    def _fit_adaptiveMCMC_normal_known_sigma(self, X: np.ndarray, y: np.ndarray, acceptance_rate: float,
        ...
        if self.verbose:
            # verbose가 True일 경우에만 progress bar 출력
            iters = tqdm(range(self.mcmcsize), desc=f"AdaptiveMCMC Normal Known Sigma Chain {chain_idx + 1}")
        else:
            iters = range(self.mcmcsize)
    ...
    def _autocorrelation(self, samples: np.ndarray, lag: int) -> float:
        ...
        denominator = np.sum((samples - mean) ** 2)  # 분모 계산
    ...
    def _calculate_summary_stats(self, burn_in: int = 0) -> dict:
        ...
        """
        # 모든 체인의 베타를 결합
        combined_beta = np.vstack([chain[burn_in:self.trueindex[chain_idx], :] for chain_idx, chain in enumerate(self.beta)])
