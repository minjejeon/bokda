...
class HLW:
    """
    이 클래스는 HLW 모델을 사용하여 주어진 경제 데이터를 기반으로 중립금리를 추정합니다.
    ...
    MergedPriorPara : dict
        ...
        - 'sig2_g_shape', 'sig2_g_scale', 'sig2_yt_shape', 'sig2_yt_scale', 'sig2_z_shape', 'sig2_z_scale', 'sig2_p_shape', 'sig2_p_scale': float
    ...
    ParaStorage : dict
        ...
        - 'sig2_yc', 'sig2_yt', 'sig2_g', 'sig2_p', 'sig2_z': numpy.ndarray, shape (numMCMC, 1)
            해당 분산 파라미터들의 샘플들.
    ...
    def __init__(self,
        ...
        # model parameters (11 parameters)
        # output cycle
        DefaultInitialPara = {
            'phi1_yc': 1.2,
            'phi2_yc': -0.5,
    ...
    def fit(self, data, dates=None):
        ...
        data : pandas.DataFrame
            ...
            컬럼 구성은 반드시 다음과 같아야 합니다. [ln real gdp, inflation rate, real interest rate]
        ...
        2. 모델을 적합시키기 위해 내부 메서드 `_fit_hlw`를 호출합니다.
        ...
        if data.shape[1] != col_size:
            # just warn 
            print(f"> Warning: Input data must have {col_size} columns. (Column Order: [ln real gdp, inflation rate, real interest rate])")

        # Extract the data columns (ln real GDP, (annualized) inflation rate, (annualized) real interest rate)
        if isinstance(data, pd.DataFrame):
            Data_full = data.to_numpy()
        else:
            Data_full = np.array(data)
        # Data_full = np.array(data.iloc[:, 1:])  # This will extract all data columns (excluding the Date)
        # Data_full = np.arr

        # construct dependent variable vectors
        Y = Data_full[4:, :2]  # output and inflation rate
        X = np.column_stack((
        ...
        ### FIT ###
        ParaStorage, SummaryParas, LVariableStorage, SummaryLV, Accept_ratio = self._fit_hlw(Data_full, Y, X, y_data, p_data, r_data)
    ...
    def print_results(self):
        ...
        Notes
        -----
        잠재 변수들에 대한 분포의 중앙값과 밴드를 그래프로 출력합니다.
    ...
    def _fit_hlw(self, Data_full, Y, X, y_data, p_data, r_data):
        """
        HLW 모델을 사용하여 데이터에 적합하고, 파라미터 및 잠재 변수의 샘플을 생성합니다.

        Parameters
        ----------
        Data_full : numpy.ndarray, shape(N_T,4)
            전체 입력 데이터 배열입니다.
        Y : numpy.ndarray, shape(N_T-4)
            종속 변수 행렬입니다.
        X : numpy.ndarray, shape(N_T-4,8)
            독립 변수 행렬입니다.
        ...
        #####################################################################################
        #
        # Estimation begins here. Given Data: r_data, y_data, p_data
        #
        #####################################################################################
        ...
        # NOTE: 가장 최신의 loglikelihood값만을 저장
        last_loglik = {
            "sig2_g": None,
            "sig2_yt": None,
            "sig2_yc": None,
        ...
        # 반복문 시작
        if self.verbose:
            # iterator = tqdm(range(1, self.numMCMC + self.burnin + 1), desc="MCMC Sampling")
        ...
        for itr in iterator:
            ...
            bt_draw = func.GenerateLatentVariables_HLW(Y, X, ParaSample, PriorLatent, SizeInformation)
            ...
            LVariableSample = {
                ...
                'z': bt_draw[:, 6],  # natural rate z component (t = 0 ~T)ß
            }
            ...
            if RunningMCMCAlgorithm['sig2_g'] == 0:
                ...
                ParaSample['sig2_g'], MHSample['sig2_g'], accept_idx_sig2_g, last_loglik['sig2_g'] = func.AdaptiveMH_IG(
                    Y, X, 'sig2_g', 'HLW', ProposalType_idx, MHSample['sig2_g'], ParaSample,
            ...
            if RunningMCMCAlgorithm['sig2_yt'] == 0:
                ...
                ParaSample['sig2_yt'], MHSample['sig2_yt'], accept_idx_sig2_yt, last_loglik['sig2_yc'] = func.AdaptiveMH_IG(
                    Y, X, 'sig2_yt', 'HLW', ProposalType_idx, MHSample['sig2_yt'], ParaSample,
            ...
            if RunningMCMCAlgorithm['sig2_yc'] == 0:
                ...
                ParaSample['sig2_yc'], MHSample['sig2_yc'], accept_idx_sig2_yc, last_loglik['sig2_yc'] = func.AdaptiveMH_IG(
                    Y, X, 'sig2_yc', 'HLW', ProposalType_idx, MHSample['sig2_yc'], ParaSample,
            ...
            if RunningMCMCAlgorithm['slope_yc'] == 0:
                ...
                yc_coeff_draw, MHSample['slope_yc'], accept_idx_slope_yc, last_loglik['slope_yc'] = func.AdaptiveMH_Normal(
                    Y, X, 'yc', 'HLW', ProposalType_idx, MHSample['slope_yc'], ParaSample,
            ...
            if RunningMCMCAlgorithm['sig2_z'] == 0:
                ...
                ParaSample['sig2_z'], MHSample['sig2_z'], accept_idx_sig2_z, last_loglik['sig2_z'] = func.AdaptiveMH_IG(
                    Y, X, 'sig2_z', 'HLW', ProposalType_idx, MHSample['sig2_z'], ParaSample,
            ...
            if self.ZSpec == 'rw':
                ...
                if RunningMCMCAlgorithm['slope_z'] == 0:
                    ...
                    z_coeff_draw, MHSample['slope_z'], accept_idx_slope_z, last_loglik['slope_z'] = func.AdaptiveMH_Normal(
                        Y, X, 'zc', 'HLW', ProposalType_idx, MHSample['slope_z'], ParaSample,
                ...
                if RunningMCMCAlgorithm['slope_z'] == 0:
                    ...
                    # MH sampling
                    z_coeff_draw, MHSample['slope_z'], accept_idx_slope_z, last_loglik['slope_z'] = func.AdaptiveMH_Normal(
                        Y, X, 'z', 'HLW', ProposalType_idx, MHSample['slope_z'], ParaSample,
                        PriorLatent, self.MergedPriorPara['z_mean'], self.MergedPriorPara['z_variance'], self.MergedPriorPara['z_logdet_variance'], ProposalVar_0_Normal, SizeInformation,
                        last_loglik['slope_z'],
                    )
                ParaSample['delta_z'] = 0
                ParaSample['phi_z'] = z_coeff_draw[0]  # NOTE: z_coeff_draw는 [value,] 형태이므로 0번 가져옴
            ...
            if RunningMCMCAlgorithm['sig2_p'] == 0:
                ...
                ParaSample['sig2_p'], MHSample['sig2_p'], accept_idx_sig2_p, last_loglik['sig2_p'] = func.AdaptiveMH_IG(
                    Y, X, 'sig2_p', 'HLW', ProposalType_idx, MHSample['sig2_p'], ParaSample,
            ...
            if RunningMCMCAlgorithm['slope_p'] == 0:
                ...
                p_coeff_draw, MHSample['slope_p'], accept_idx_slope_p, last_loglik['slope_p'] = func.AdaptiveMH_Normal(
                    Y, X, 'p', 'HLW', ProposalType_idx, MHSample['slope_p'], ParaSample,
            ...
            if itr > self.burnin:
                ...
                ParaStorage['phi_yc'][itr_idx] = np.array([ParaSample['phi1_yc'], ParaSample['phi2_yc']]).reshape(2)  # NOTE: shape 맞추기 위한 수정 (2, 1) > (2,)
        ...
        if self.MergedMCMCAlgorithm['sig2_g'] == 1:
            Accept_ratio['sig2_g'] = np.mean(Accept_idx['sig2_g'])
        if self.MergedMCMCAlgorithm['sig2_yt'] == 1:
            Accept_ratio['sig2_yt'] = np.mean(Accept_idx['sig2_yt'])
        if self.MergedMCMCAlgorithm['sig2_yc'] == 1:
            Accept_ratio['sig2_yc'] = np.mean(Accept_idx['sig2_yc'])
        if self.MergedMCMCAlgorithm['sig2_z'] == 1:
            Accept_ratio['sig2_z'] = np.mean(Accept_idx['sig2_z'])
        if self.MergedMCMCAlgorithm['sig2_p'] == 1:
            Accept_ratio['sig2_p'] = np.mean(Accept_idx['sig2_p'])
        if self.MergedMCMCAlgorithm['slope_yc'] == 1:
            Accept_ratio['slope_yc'] = np.mean(Accept_idx['slope_yc'])
        if self.MergedMCMCAlgorithm['slope_z'] == 1:
            Accept_ratio['slope_z'] = np.mean(Accept_idx['slope_z'])
        if self.MergedMCMCAlgorithm['slope_p'] == 1:
            Accept_ratio['slope_p'] = np.mean(Accept_idx['slope_p'])
