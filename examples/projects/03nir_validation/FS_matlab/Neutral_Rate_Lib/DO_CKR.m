...
% ** Vinv: -Hessian
function [mu,fmax,g,V,Vinv] = DO_CKR(FUN,CONSTR,arg,index,maxiter,co,printi,Sn)

argnew = arg;

if co == 1
    valid = CONSTR(argnew);  %/* check if parameter constraints are valid for initial value */
...
while db > 1e-6 && iter <= maxiter && s >= 1e-6;  
    ...
    if co == 1 && CONSTR(x00) == 0     % if constraint is not satisfied, reduce the stepsize */

    	while s > 0 && CONSTR(x00) == 0
    	    x00 = argnew;
         	x00(index) = x00(index) + s*db/2;
