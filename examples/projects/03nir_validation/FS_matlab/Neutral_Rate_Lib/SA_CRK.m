...
function [mu,fmax,g,V,Vinv] = SA_CRK(FUN,constr,SF,arg,index,a,IT,b,cs,IM,n,mr,eps,maxiter,co,printi,Sn) 
...
reject = 0;
rate = zeros(1,n);
acceptm = zeros(1,n);   %/* total number of M-H acceptances in each stage */
...
while j <= n;   % /* stage indicator */
    ...
    while i <= mj && reject <=mr;
        ...
        if co == 1;     %/* constrained problem */
            ...
            while valid == 0;
                ...
                newparamj(ind) = currentj(ind) + randn(1,1)/SF; %/* proposed move */ 
    ...
    if rate(:,j) > 0.7
        SF = SF/(1 + cs*(rate(:,j) - 0.7)/.3);
    elseif (rate(:,j) < .2)
        SF = SF*(1 + cs*((0.2 - rate(:,j))/0.2));
    end
