         ...
         ::coder::array<real_T, 2U> &y)
{
  ...
  if ((x.size(0) == 0) || (x.size(1) == 0)) {
    int32_T b_n;
    y.set_size(&nb_emlrtRTEI, sp, x.size(0), x.size(1));
    ...
    st.site = &pb_emlrtRSI;
    n = x.size(0);
    y.set_size(&ib_emlrtRTEI, &st, x.size(0), x.size(1));
    b_n = x.size(0) * x.size(1);
    for (i = 0; i < b_n; i++) {
      y[i] = 0.0;
    }
    b_st.site = &rb_emlrtRSI;
    b_x.set_size(&jb_emlrtRTEI, &b_st, x.size(0), x.size(1));
    b_n = x.size(0) * x.size(1);
    for (i = 0; i < b_n; i++) {
      b_x[i] = x[i];
    }
    ...
    b_n = x.size(0);
    p.set_size(&mb_emlrtRTEI, &f_st, 1, x.size(0));
    p[0] = 1;
    yk = 1;
