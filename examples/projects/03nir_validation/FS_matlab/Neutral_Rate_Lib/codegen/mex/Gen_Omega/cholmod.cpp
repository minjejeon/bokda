             ...
             coder::array<real_T, 2U> &R)
{
  ...
  L.set_size(&db_emlrtRTEI, sp, A.size(0), A.size(1));
  last = A.size(1);
  for (i = 0; i < last; i++) {
    idx = A.size(0);
    for (i1 = 0; i1 < idx; i1++) {
  ...
  st.site = &r_emlrtRSI;
  L.set_size(&eb_emlrtRTEI, &st, A.size(0), A.size(1));
  ...
  K.set_size(&fb_emlrtRTEI, &g_st, 1, varargin_1.size(1));
  ...
  d_st.site = &wb_emlrtRSI;
  e_st.site = &xb_emlrtRSI;
  K.set_size(&gb_emlrtRTEI, &e_st, 1, 3);
  K[0] = b_gamma;
  K[1] = theta;
  ...
  //  initialize d and L
  b_d.set_size(&hb_emlrtRTEI, sp, n);
  for (i = 0; i < n; i++) {
    b_d[i] = 0.0;
  }
  st.site = &u_emlrtRSI;
  ...
  for (j = 0; j < n; j++) {
    real_T djtemp;
    if (j < 1) {
      K.set_size(&ib_emlrtRTEI, sp, 1, 0);
    } else {
      K.set_size(&ib_emlrtRTEI, sp, 1, j);
      last = j - 1;
      for (i = 0; i <= last; i++) {
    ...
    r.set_size(&jb_emlrtRTEI, sp, K.size(1));
    last = K.size(1);
    for (i = 0; i < last; i++) {
      r[i] = K[i];
    }
    ...
    K.set_size(&lb_emlrtRTEI, &st, 1, r.size(0));
    last = r.size(0);
    for (i = 0; i < last; i++) {
      i1 = static_cast<int32_T>(r[i]);
      if (i1 > L.size(1)) {
    ...
    b_varargin_1.set_size(&mb_emlrtRTEI, &st, r.size(0));
    last = r.size(0);
    for (i = 0; i < last; i++) {
    ...
    if (j + 1 < n) {
      ...
      if (static_cast<uint32_T>(n) < j + 2U) {
        K.set_size(&nb_emlrtRTEI, &b_st, 1, 0);
      } else {
        i = n - j;
        K.set_size(&nb_emlrtRTEI, &b_st, 1, i - 1);
        last = i - 2;
        for (i = 0; i <= last; i++) {
      ...
      r1.set_size(&pb_emlrtRTEI, sp, K.size(1));
      last = K.size(1);
      for (i = 0; i < last; i++) {
        r1[i] = K[i];
      }
      ...
      st.site = &x_emlrtRSI;
      varargin_1.set_size(&qb_emlrtRTEI, &st, r1.size(0), r.size(0));
      last = r.size(0);
      for (i = 0; i < last; i++) {
        idx = r1.size(0);
        for (i1 = 0; i1 < idx; i1++) {
      ...
      b_varargin_1.set_size(&rb_emlrtRTEI, &st, r.size(0));
      last = r.size(0);
      for (i = 0; i < last; i++) {
        b_varargin_1[i] = b_d[static_cast<int32_T>(r[i]) - 1] * diagA[i];
      }
      ...
      diagA.set_size(&sb_emlrtRTEI, sp, r1.size(0));
      last = r1.size(0);
      for (i = 0; i < last; i++) {
      ...
      r2.set_size(&tb_emlrtRTEI, sp, r1.size(0));
      last = r1.size(0);
      for (i = 0; i < last; i++) {
        i1 = static_cast<int32_T>(r1[i]);
        if ((i1 < 1) || (i1 > L.size(0))) {
  ...
  for (j = 0; j < n; j++) {
    ...
    last = L.size(0);
    r.set_size(&ub_emlrtRTEI, sp, L.size(0));
    for (i = 0; i < last; i++) {
  ...
  R.set_size(&vb_emlrtRTEI, sp, L.size(1), L.size(0));
  last = L.size(0);
  for (i = 0; i < last; i++) {
    idx = L.size(1);
    for (i1 = 0; i1 < idx; i1++) {
