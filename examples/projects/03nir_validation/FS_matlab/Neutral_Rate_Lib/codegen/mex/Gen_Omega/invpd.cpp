...
static emlrtRSInfo l_emlrtRSI{ 22, // lineNo
"invpd", // fcnName
"D:\\Dropbox\\Policy_Works\\NL_LBVAR_2023\\Codes\\M_library\\invpd.m"// pathName
 };
...
"eye", // fcnName
"C:\\Program Files\\MATLAB\\R2021a\\toolbox\\eml\\lib\\matlab\\elmat\\eye.m"// pathName
 };
...
static emlrtRTEInfo x_emlrtRTEI{ 23, // lineNo
18, // colNo
"invuptr", // fName
"D:\\Dropbox\\Policy_Works\\NL_LBVAR_2023\\Codes\\M_library\\invuptr.m"// pName
 };
...
static emlrtRTEInfo bb_emlrtRTEI{ 25, // lineNo
5, // colNo
"invpd", // fName
"D:\\Dropbox\\Policy_Works\\NL_LBVAR_2023\\Codes\\M_library\\invpd.m"// pName
 };
...
static emlrtRSInfo wc_emlrtRSI{ 11, // lineNo
"invpd", // fcnName
"D:\\Dropbox\\Policy_Works\\NL_LBVAR_2023\\Codes\\M_library\\invpd.m"// pathName
 };

static emlrtRSInfo xc_emlrtRSI{ 11, // lineNo
"invuptr", // fcnName
"D:\\Dropbox\\Policy_Works\\NL_LBVAR_2023\\Codes\\M_library\\invuptr.m"// pathName
 };

static emlrtRSInfo yc_emlrtRSI{ 17, // lineNo
"invuptr", // fcnName
"D:\\Dropbox\\Policy_Works\\NL_LBVAR_2023\\Codes\\M_library\\invuptr.m"// pathName
 };
...
void invpd(const emlrtStack *sp, const coder::array<real_T, 2U> &A, coder::array<real_T, 2U> &Ainv)
{
    ...
    if (A.size(0) != A.size(1)) {
        ...
        st.site = &wc_emlrtRSI;
        disp(&st, y, &emlrtMCI);
        Ainv.set_size(&cb_emlrtRTEI, sp, 0, 0);
    } else {
        int32_T b_loop_ub;
        int32_T i;
        int32_T i1;
        ...
        if (Hinv.size(0) != n) {
            ...
            emlrtForLoopVectorCheckR2021a(static_cast<real_T>(n), -1.0, 1.0, mxDOUBLE_CLASS, i, &d_emlrtRTEI, &st);
            k = 0;
            exitg1 = false;
            while ((!exitg1) && (k <= i - 1)) {
                int32_T b_k;
                ...
                if (sum == 0.0) {
                    ...
                    if (b_k > Hinv.size(1)) {
                        emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(1), &e_emlrtBCI, &st);
                    }
                    if (b_k > Hinv.size(0)) {
                        emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(0), &e_emlrtBCI, &st);
                    }
                    if (b_k > Hinv.size(1)) {
                        emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(1), &f_emlrtBCI, &st);
                    }
                    if (b_k > Hinv.size(0)) {
                        emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(0), &f_emlrtBCI, &st);
                    }
                    Hinv[(b_k + Hinv.size(0) * (b_k - 1)) - 1] = 1.0 / sum;
                    i1 = static_cast<int32_T>(((-1.0 - (static_cast<real_T>(b_k) - 1.0)) + 1.0) / -1.0);
                    emlrtForLoopVectorCheckR2021a(static_cast<real_T>(b_k) - 1.0, -1.0, 1.0, mxDOUBLE_CLASS, i1, &e_emlrtRTEI, &st);
                    for ( int32_T b_i{0}; b_i < i1; b_i++) {
                        ...
                        if (c_i > b_k) {
                            ...
                            if ((c_i < 1) || (c_i > Hinv.size(1))) {
                                emlrtDynamicBoundsCheckR2012b(c_i, 1, Hinv.size(1), &i_emlrtBCI, &st);
                            }
                            i2 = c_i - 2;
                            if (b_k > Hinv.size(1)) {
                                emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(1), &j_emlrtBCI, &st);
                            }
                            i3 = b_k - 1;
                            if (c_i > Hinv.size(0)) {
                                emlrtDynamicBoundsCheckR2012b(c_i, 1, Hinv.size(0), &k_emlrtBCI, &st);
                            }
                            i4 = c_i - 2;
                            if (b_k > Hinv.size(0)) {
                                emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(0), &l_emlrtBCI, &st);
                            }
                            i5 = b_k - 1;
                        }
                        b_st.site = &rc_emlrtRSI;
                        if ((c_i - 1 < 1) || (c_i - 1 > Hinv.size(0))) {
                            emlrtDynamicBoundsCheckR2012b(c_i - 1, 1, Hinv.size(0), &h_emlrtBCI, &b_st);
                        }
                        loop_ub = i3 - i2;
                        a.set_size(&x_emlrtRTEI, &b_st, 1, loop_ub);
                        for (b_loop_ub = 0; b_loop_ub < loop_ub; b_loop_ub++) {
                            a[b_loop_ub] = Hinv[(c_i + Hinv.size(0) * ((i2 + b_loop_ub) + 1)) - 2];
                        }
                        if (b_k > Hinv.size(1)) {
                            emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(1), &m_emlrtBCI, &b_st);
                        }
                        b_loop_ub = i5 - i4;
                        b.set_size(&y_emlrtRTEI, &b_st, b_loop_ub);
                        ...
                        if (b_k > Hinv.size(1)) {
                            emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(1), &c_emlrtBCI, &st);
                        }
                        if ((c_i - 1 < 1) || (c_i - 1 > Hinv.size(0))) {
                            emlrtDynamicBoundsCheckR2012b(c_i - 1, 1, Hinv.size(0), &c_emlrtBCI, &st);
                        }
                        if ((c_i - 1 < 1) || (c_i - 1 > Hinv.size(1))) {
                            emlrtDynamicBoundsCheckR2012b(c_i - 1, 1, Hinv.size(1), &d_emlrtBCI, &st);
                        }
                        if ((c_i - 1 < 1) || (c_i - 1 > Hinv.size(0))) {
                            emlrtDynamicBoundsCheckR2012b(c_i - 1, 1, Hinv.size(0), &d_emlrtBCI, &st);
                        }
                        Hinv[(c_i + Hinv.size(0) * (b_k - 1)) - 2] = -sum / Hinv[(c_i + Hinv.size(0) * (c_i - 2)) - 2];
        ...
        coder::internal::blas::mtimes(&b_st, Hinv, Hinv, Ainv);
        ...
        Hinv.set_size(&ab_emlrtRTEI, sp, Ainv.size(0), Ainv.size(1));
        loop_ub = Ainv.size(1);
        for (i = 0; i < loop_ub; i++) {
            b_loop_ub = Ainv.size(0);
            for (i1 = 0; i1 < b_loop_ub; i1++) {
                Hinv[i1 + Hinv.size(0) * i] = (Ainv[i1 + Ainv.size(0) * i] + Ainv[i + Ainv.size(0) * i1]) / 2.0;
            }
        }
        Ainv.set_size(&bb_emlrtRTEI, sp, Hinv.size(0), Hinv.size(1));
