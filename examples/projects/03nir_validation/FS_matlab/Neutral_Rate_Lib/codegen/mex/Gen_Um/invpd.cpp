...
static emlrtBCInfo emlrtBCI{
    ...
    "T",       // aName
    "invuptr", // fName
    "D:\\Dropbox\\Policy_Works\\KOR_US_NIR\\Codes2\\Neutral_Rate_Lib\\invuptr."
    "m", // pName
    0    // checkKind
};
           ...
           coder::array<real_T, 2U> &Ainv)
{
  ...
  if (A.size(0) != A.size(1)) {
    ...
    st.site = &od_emlrtRSI;
    disp(&st, y, &emlrtMCI);
    Ainv.set_size(&u_emlrtRTEI, sp, 0, 0);
  } else {
    int32_T b_loop_ub;
    int32_T i;
    int32_T i1;
    ...
    if (Hinv.size(0) != n) {
      ...
      while ((!exitg1) && (k <= i - 1)) {
        ...
        if (sum == 0.0) {
          ...
          if (b_k > Hinv.size(0)) {
            emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(0), &h_emlrtBCI,
                                          &st);
          }
          if (b_k > Hinv.size(1)) {
          ...
          emlrtForLoopVectorCheckR2021a(static_cast<real_T>(b_k) - 1.0, -1.0,
                                        1.0, mxDOUBLE_CLASS, i1, &f_emlrtRTEI,
                                        &st);
          for (int32_T b_i{0}; b_i < i1; b_i++) {
            ...
            if (c_i > b_k) {
              ...
              if ((c_i < 1) || (c_i > Hinv.size(1))) {
                emlrtDynamicBoundsCheckR2012b(c_i, 1, Hinv.size(1), &b_emlrtBCI,
                                              &st);
              }
              i2 = c_i - 2;
              if (b_k > Hinv.size(1)) {
              ...
              i3 = b_k - 1;
              if (c_i > Hinv.size(0)) {
                emlrtDynamicBoundsCheckR2012b(c_i, 1, Hinv.size(0), &d_emlrtBCI,
                                              &st);
              }
              i4 = c_i - 2;
              if (b_k > Hinv.size(0)) {
            ...
            if ((c_i - 1 < 1) || (c_i - 1 > Hinv.size(0))) {
              emlrtDynamicBoundsCheckR2012b(c_i - 1, 1, Hinv.size(0), &emlrtBCI,
                                            &b_st);
            }
            loop_ub = i3 - i2;
            a.set_size(&q_emlrtRTEI, &b_st, 1, loop_ub);
            ...
            b_loop_ub = i5 - i4;
            b.set_size(&r_emlrtRTEI, &b_st, b_loop_ub);
    ...
    coder::internal::blas::b_mtimes(&b_st, Hinv, Hinv, Ainv);
    b_Ainv[0] = Ainv.size(1);
    b_Ainv[1] = Ainv.size(0);
    iv2[0] = (*(int32_T(*)[2])Ainv.size())[0];
    iv2[1] = (*(int32_T(*)[2])Ainv.size())[1];
    emlrtSizeEqCheckNDR2012b(&iv2[0], &b_Ainv[0], &emlrtECI, (emlrtCTX)sp);
    Hinv.set_size(&s_emlrtRTEI, sp, Ainv.size(0), Ainv.size(1));
    loop_ub = Ainv.size(1);
    for (i = 0; i < loop_ub; i++) {
    ...
    Ainv.set_size(&t_emlrtRTEI, sp, Hinv.size(0), Hinv.size(1));
