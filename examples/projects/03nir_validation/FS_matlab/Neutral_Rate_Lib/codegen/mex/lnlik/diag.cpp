          ...
          ::coder::array<real_T, 2U> &d)
{
  ...
  b_st.tls = st.tls;
  nv = v.size(0);
  d.set_size(&lb_emlrtRTEI, sp, v.size(0), v.size(0));
  ...
  if ((v.size(0) == 1) && (v.size(1) == 1)) {
    ...
    if ((v.size(0) == 1) || (v.size(1) == 1)) {
      emlrtErrorWithMessageIdR2018a(
          sp, &q_emlrtRTEI, "Coder:toolbox:diag_varsizedMatrixVector",
