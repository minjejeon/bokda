           ...
           coder::array<real_T, 2U> &Ainv)
{
  ...
  if (A.size(0) != A.size(1)) {
    ...
    st.site = &vd_emlrtRSI;
    disp(&st, y, &emlrtMCI);
    Ainv.set_size(&sb_emlrtRTEI, sp, 0, 0);
  } else {
    int32_T b_loop_ub;
    int32_T i;
    int32_T i1;
    ...
    if (Hinv.size(0) != n) {
      ...
      emlrtForLoopVectorCheckR2021a(static_cast<real_T>(n), -1.0, 1.0,
                                    mxDOUBLE_CLASS, i, &k_emlrtRTEI, &st);
      k = 0;
      exitg1 = false;
      while ((!exitg1) && (k <= i - 1)) {
        int32_T b_k;
        ...
        if ((b_k < 1) || (b_k > Hinv.size(0))) {
          emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(0), &x_emlrtBCI, &st);
        }
        if (b_k > Hinv.size(1)) {
          emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(1), &x_emlrtBCI, &st);
        }
        ...
        if (sum == 0.0) {
          ...
          emlrtForLoopVectorCheckR2021a(static_cast<real_T>(b_k) - 1.0, -1.0,
                                        1.0, mxDOUBLE_CLASS, i1, &l_emlrtRTEI,
                                        &st);
          for (int32_T b_i{0}; b_i < i1; b_i++) {
            ...
            if (c_i > b_k) {
              ...
              if ((c_i < 1) || (c_i > Hinv.size(1))) {
                emlrtDynamicBoundsCheckR2012b(c_i, 1, Hinv.size(1), &s_emlrtBCI,
                                              &st);
              }
              i2 = c_i - 2;
              if (b_k > Hinv.size(1)) {
                emlrtDynamicBoundsCheckR2012b(b_k, 1, Hinv.size(1), &t_emlrtBCI,
                                              &st);
              }
              i3 = b_k - 1;
              if (c_i > Hinv.size(0)) {
                emlrtDynamicBoundsCheckR2012b(c_i, 1, Hinv.size(0), &u_emlrtBCI,
                                              &st);
              }
              i4 = c_i - 2;
              if (b_k > Hinv.size(0)) {
            ...
            loop_ub = i3 - i2;
            a.set_size(&ob_emlrtRTEI, &b_st, 1, loop_ub);
            ...
            b_loop_ub = i5 - i4;
            b.set_size(&pb_emlrtRTEI, &b_st, b_loop_ub);
    ...
    emlrtSizeEqCheckNDR2012b(&iv2[0], &b_Ainv[0], &p_emlrtECI, (emlrtCTX)sp);
    Hinv.set_size(&qb_emlrtRTEI, sp, Ainv.size(0), Ainv.size(1));
    loop_ub = Ainv.size(1);
    for (i = 0; i < loop_ub; i++) {
    ...
    Ainv.set_size(&rb_emlrtRTEI, sp, Hinv.size(0), Hinv.size(1));
